...
export function sha1(x: string) {
    const hash = createHash('sha1');
    hash.update(x);
    return hash.digest('hex');
}

export function createSalt(byteCount: number) {
...
export function hashRedminePassword(clearPassword: string, salt?: string) {
    const hashed_password = salt ? sha1(`${salt}${sha1(clearPassword)}`) : sha1(clearPassword);
    return hashed_password;
}

export function verifyPassword(user: Record<string,any>, clearPassword: string) {
    if (!user) {
        return false;
    }
...
export function hashPasswordForStorage(clearPassword: string, salt?: string) {
