...
export function getUserAndPassFromBasicAuth(headers: Headers) {
...
export async function verifyBasicAuth(db: TransactionOrKnex, headers: Headers) {
...
export function makeJwt(username: string) {
    var token = jwt.sign({
        sub: username,
        aud: jwtAudience,
    ...
    return new Promise((resolve, reject) => {
        jwt.verify(token, signKey, { audience: jwtAudience, algorithms: ['HS256'] }, function(err, payload) {
            if (err) {
                ...
                resolve(payload as Promise<jwt.JwtPayload>);
            }
        });
    });
}

export async function verifyUsernameFromToken(token: string) {
    try {
...
export function getJwtTokenFromAuthHeaders(headers: Headers) {
    if (headers && headers.has('authorization')) {
        const auth = headers.get('authorization');
...
export async function verifyJwtAuth(db: TransactionOrKnex, headers: Headers) {
