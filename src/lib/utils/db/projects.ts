...
export function allProjectsQuery(db: TransactionOrKnex, { limit = undefined, offset = undefined } = {}) {
...
export function countAllProjectsQuery(db: TransactionOrKnex, params: any) {
    return allProjectsQuery(db, params).resultSize();
}

export function getAllProjects(db: TransactionOrKnex, params: any) {
...
export function oneProjectQuery(db: TransactionOrKnex, projectCode: string) {
    return Project.query(db).where('identifier', projectCode);
}

export function getOneProject(db: TransactionOrKnex, projectCode: string) {
    const query = oneProjectQuery(db, projectCode).withGraphJoined('members.[user, role]');
    return onlyOne(query, 'projectCode', 'project code', (project: any) => {
...
export async function createOneProject(db: TransactionOrKnex, projectCode: string, newProject: Record<string, any>, headers: Headers) {
    ...
    async () => {
        ...
        const subResult = await addUserWithRole(trx, project, (authUser as any).login, managerRoleId);
...
export async function patchOneProject(db: TransactionOrKnex, projectCode: string, updateData: Record<string, any>) {
...
export async function deleteOneProject(db: TransactionOrKnex, projectCode: string) {
    const trx = await Project.startTransaction(db);
    const query = oneProjectQuery(trx, projectCode).select('id').forUpdate();
    const result = await atMostOne(query, 'projectCode', 'project code',
    () => {
    ...
    async (project: any) => {
        ...
        const membershipIds = (await retryOnServerError(Membership.query(trx).where({project_id: project.id}).select('id'))).map((m: any) => m.id);
