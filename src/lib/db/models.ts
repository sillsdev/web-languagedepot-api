import { Model, type ModelOptions, type Pojo, type QueryContext } from 'objection';
...
class User extends Model {
    ...
    async $beforeInsert(context: QueryContext) {
        ...
        if (Object.prototype.hasOwnProperty.call(this, 'password')) {
            const { hashed_password, salt } = hashPasswordForStorage((this as any).password, undefined);
            (this as any).hashed_password = hashed_password;
            (this as any).salt = salt;
    ...
    async $beforeUpdate(opt: ModelOptions, context: QueryContext) {
        ...
        setDateColumnsForUpdate(this);
        if (Object.prototype.hasOwnProperty.call(this, 'password')) {
            if ((this as any).password) { // Empty password in incoming JSON will leave existing password unchanged
                const { hashed_password, salt } = hashPasswordForStorage((this as any).password, undefined);
                (this as any).hashed_password = hashed_password;
                (this as any).salt = salt;
            }
